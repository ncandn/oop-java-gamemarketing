	...
	public ProfileUI(JFrame menuFrame, Management mng) {
		...
		btn_SaveChanges.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				{
					String oldUserNameString = ((User) mng.getActiveUser()).getUser_name();
					...
					try {
						mng.getSocial().getUserMap().put(((User)mng.getActiveUser()).getUser_name(), (User)mng.getActiveUser());
						mng.getSocial().getUserMap().remove(oldUserNameString);
						mng.updateUser();
						mng.updateSocial();
						mng.updatelibrary();
